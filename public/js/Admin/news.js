admin.news = {
    initialize: function ()
    {
        function readURL(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    $('#news_image_priview').attr('src', e.target.result);
                }
        ...
        $('body').on('click', '.btnDelete_news', function () {
            var id = $(this).data('id');
            this_class.delete_row(id);
        });

        admin.news.load_news();
        admin.news.refresh_validator();
    ...
    load_news: function () {
        ...
        var table = jQuery('.news-table').DataTable({
            paging: true,
            pageLength: 10,
            bDestroy: true,
            responsive: true,
...
//                {data: 'description', name: 'description'},
                ...
                {data: 'news_image', name: 'news_image' ,orderable: false, searchable: false},
