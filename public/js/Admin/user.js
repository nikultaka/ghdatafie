        ...
        $("body").on("change", "#email", function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.status == 1){
                        $('#email').attr('style', 'border-color: rgb(185, 74, 72) !important');
                        $('#email').addClass('error'); 
                        $('#email').attr('data-validation-error-msg', data.msg);
...
//                        $("#role_name").children("option:selected").val(data.content.role_id);
                        $('select[name^="role_name"] option[value="'+data.content.role_id+'"]').attr("selected","selected");
