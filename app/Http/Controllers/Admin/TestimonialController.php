...
//            }
            if (isset($row->user_photo) && $row->user_photo != '') {
                $media = ASSET . 'upload/testimonial/thumbnail/' . $row->user_photo;
            } else {
                $media = ASSET . 'upload/' . NO_IMAGE_AVAILABLE;
            }
            $temp['id'] = $row->id;
...
//            $temp['user_photo'] = $media;
            ...
            $temp['user_photo'] = '<div><img style="width: 50px;" src="'.url($media).'"></div>';
            
            $id = $row->id;

            $action = '<div class="datatable_btn" style="display:flex;"><a href="javascript:void(0);" data-id="' . $id . '" class="btn btn-xs btn-info btnEdit_test"> Edit</a>  	&nbsp;';
