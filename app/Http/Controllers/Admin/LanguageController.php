...
class LanguageController extends Controller
{
    ...
    public function add(Request $request){
        ...
        if (!empty($post)) {
            ...
            if (isset($post['id']) && $post['id'] != '') {
                ...
                $returnresult = DB::table('language')
                        ->where('id', $id)
                        ->update($data);
                ...
                if (DB::table('language')->insert($data)) {
                    $result['status'] = 1;
    ...
    public function delete(Request $request){
     ...
     public function data_table() {
        ...
        $select_query = DB::table('language')
                ->where('status', '!=', -1);
        ...
        foreach ($banner_list as $row) {
           ...
           $action = '<div class="datatable_btn"><a href="javascript:void(0);" data-id="' . $id . '" class="btn btn-xs btn-info btnEdit_language"> Edit</a>  	&nbsp;';
            $action .= '<a href="javascript:void(0);" data-id="' . $id . '" type="button" class="btn btn-xs btn-danger btnDelete_language "> Delete</a></div>';
    ...
    public function change(Request $request){
        ...
        $data_result['status'] = 0;
         if (!empty($post)) {
             $data['admin']['language'] = $post['language_id'];
             session()->put($data);
            // $data_result['status'] = 1;
         }
