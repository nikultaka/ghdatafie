...
//        $data_result='';
    ...
    public function delete(Request $request){
     ...
     public function user_data_table() {
        ...
        $select_query = DB::table('users as u')->where('u.status', '!=', -1);

        $select_query->select('u.*', DB::raw("IF(u.status = 1,'Active','Inactive') as status"),'ur.type');
        
        $select_query->leftjoin('user_role as ur','ur.role_id','u.role_id');

        if (isset($requestData['search']['value']) && $requestData['search']['value'] != '') {

            $select_query->where("u.name", "like", '%' . $requestData['search']['value'] . '%')

                    ->oRwhere("ur.type", "like", '%' . $requestData['search']['value'] . '%')
